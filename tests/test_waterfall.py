...
def test_nothing_outside_tests_imports_the_waterfall():
    """The proof that no published unlevered number can have moved — the
    same guard E1 shipped. Delete this test in E3, when wiring it is the
    point.
    """
    import ast
    import pathlib
    ...
    def imports_waterfall(tree):
        """Real imports only — a prose mention in a comment or docstring
        is documentation, not wiring."""
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                if any(a.name in ("model.waterfall", "waterfall")
                       for a in node.names):
                    return True
            elif isinstance(node, ast.ImportFrom):
                if node.module == "model.waterfall":
                    return True
                if node.module == "model" and any(a.name == "waterfall"
                                                  for a in node.names):
                    return True
        return False
    ...
    offenders = []
    for path in sources:
        if path.name == "waterfall.py" and path.parent.name == "model":
            continue
        tree = ast.parse(path.read_text(encoding="utf-8"), filename=str(path))
        if imports_waterfall(tree):
            offenders.append(str(path.relative_to(root)))
    assert offenders == [], f"E2 is not wired yet, but found: {offenders}"
